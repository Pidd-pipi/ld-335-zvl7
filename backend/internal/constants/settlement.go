...
const (
	...
	SettlementAdjusted      = "adjusted"       // 已补退（差额复核通过）
)
...
// AdjustmentStatus 结算差额补退单状态枚举（差额补退功能状态机）。
const (
	AdjustmentPendingReview = "pending_review" // 待复核
	AdjustmentApproved      = "approved"       // 复核通过（已补退）
	AdjustmentRejected      = "rejected"       // 复核驳回（可再次申请）
)

// AdjustmentStatuses 全部差额补退单状态。
var AdjustmentStatuses = []string{AdjustmentPendingReview, AdjustmentApproved, AdjustmentRejected}

// AdjustmentDirection 账目流水方向。
const (
	AdjustmentDirectionNegative = "negative" // 负向冲减
)
